...
// Interface helper untuk data user yang melakukan aksi (Like/Vote)
interface Actor {
    uid: string;
    displayName: string;
    photoURL: string;
...
class ForumService {
    ...
    static async createComment(input: CreateCommentInput): Promise<string> {
        ...
        // Transaction untuk simpan data & update counter
        const { commentId, postAuthorId } = await runTransaction(db, async (transaction) => {
            ...
            return { commentId: newCommentRef.id, postAuthorId: postData.authorId };
        });

        // --- 1. NOTIFIKASI KE PEMILIK POSTINGAN (Existing) ---
        if (postAuthorId !== input.authorId) {
        ...
        // --- 2. NOTIFIKASI KE PEMILIK KOMENTAR (Fitur Reply Baru) ---
        if (input.parentId) {
            try {
                // Ambil data komentar yang dibalas untuk tau siapa pemiliknya
                const parentCommentRef = doc(db, "posts", input.postId, "comments", input.parentId);
                const parentSnap = await getDoc(parentCommentRef);

                if (parentSnap.exists()) {
                    const parentData = parentSnap.data() as Comment;
                    
                    // Jangan kirim notif jika membalas komentar sendiri
                    // Dan jangan kirim double notif jika pemilik komentar == pemilik postingan (sudah dihandle di atas)
                    if (parentData.authorId !== input.authorId && parentData.authorId !== postAuthorId) {
                        await sendNotification(
                            parentData.authorId,    // Penerima (Pemilik Komentar Lama)
                            input.authorId,         // Pengirim (Yang membalas)
                            input.authorName,
                            input.authorPhoto,
                            'reply',                // Tipe Reply
                ...
                console.error("Gagal kirim notif reply:", error);
            }
        }
    ...
    static async togglePostVote(postId: string, actor: Actor, type: Vote["type"]): Promise<Vote["type"] | null> {
        const postRef = doc(db, "posts", postId);
        const voteRef = doc(postRef, "votes", actor.uid);

        const { nextVote, postAuthorId } = await runTransaction(db, async (transaction) => {
            const postSnap = await transaction.get(postRef);
            if (!postSnap.exists()) {
                throw new Error("Postingan tidak ditemukan");
            }
            ...
            return { nextVote, postAuthorId: postData.authorId };
        });

        // --- KIRIM NOTIFIKASI LIKE POST ---
        if (nextVote === 'like' && postAuthorId !== actor.uid) {
            await sendNotification(
                postAuthorId,
                actor.uid,
                actor.displayName,
    ...
    static async toggleCommentVote(
        ...
        const voteRef = doc(commentRef, "votes", actor.uid);

        const { nextVote, commentAuthorId } = await runTransaction(db, async (transaction) => {
            ...
            return { nextVote, commentAuthorId: commentData.authorId };
        });

        // --- KIRIM NOTIFIKASI LIKE KOMENTAR ---
        if (nextVote === 'like' && commentAuthorId !== actor.uid) {
            await sendNotification(
                commentAuthorId,
                actor.uid,
                actor.displayName,
